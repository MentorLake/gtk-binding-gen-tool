...
public class CSharpSignalsSerializer
{
	public static string Serialize(ClassDeclaration c, LibraryDeclaration libraryDeclaration, List<LibraryDeclaration> libraries)
	{
		...
		foreach (var signal in c.Signals)
		{
			...
			var method = @$"
	public static IObservable<{c.Name}SignalStructs.{signal.Name.ToPascalCase()}Signal> Signal_{signal.Name.ToPascalCase()}(this {c.Name}Handle instance, GConnectFlags connectFlags = GConnectFlags.G_CONNECT_AFTER)
	{{
		...
		{{
			...
			var handlerId = GObjectExterns.g_signal_connect_data(instance, ""{signal.Name}"", Marshal.GetFunctionPointerForDelegate(handler), IntPtr.Zero, null, connectFlags);
