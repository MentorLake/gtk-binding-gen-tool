...
public static class CSharpMethodSerializer
{
	...
	public static string ToInstanceMethodAdaptor(this MethodDeclaration m, string className, List<LibraryDeclaration> libraries)
	{
		...
		var returnType = ToCSharpReturnType(m);
		var externCall = $"{className}Externs.{m.Name}({string.Join(", ", m.Parameters.Select(p => p.ToCSharpArgument(libraries)))});";
		var canBeExtensionMethod = m.Parameters.Any()
			&& m.Parameters.First().ToCSharpTypeWithoutModifiers(libraries) == className + "Handle";

		if (canBeExtensionMethod)
		{
			parameters = "this " + string.Join(", ", m.Parameters.Select(a => a.ToCSharpString(libraries)));

			if (returnType == "void")
			{
				parameters = Regex.Replace(parameters, @"this ([^ ]+Handle)(.*)", $"this T$2");
				output.AppendLine($"\tpublic static T {methodName}<T>({parameters}) where T : {className}Handle");
				output.AppendLine("\t{");
