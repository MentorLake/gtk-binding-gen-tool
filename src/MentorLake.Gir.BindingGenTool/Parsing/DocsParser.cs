...
public class DocsParser
{
	...
	public static InterfaceDeclaration ParseInterfaceHtml(LibraryConfig config, string filePath)
	{
		...
		return new InterfaceDeclaration()
		{
			...
			Methods = xml
				...
				.Concat(xml
					.XPathSelectElements("body//div[@class='methods toggle-wrapper']/h4[contains(text(), 'Functions')]/..//div[@class='docblock']//h6//a")
					.Select(a => a.Attribute("href").Value)
					.Select(x => Path.Join(classDirectory, x))
					.Select(x => ParseMethodHtml(x))
					.ToList())
	...
	public static StructDeclaration ParseStruct(string filePath)
	{
		...
		decl.Methods.AddRange(xml
			.XPathSelectElements("body//div[@class='toggle-wrapper methods']/h4[contains(text(), 'Functions')]/../div[@class='docblock']/div")
			.Select(m => m.XPathSelectElement(".//a"))
			.Select(a => a.Attribute("href").Value)
			.Select(x => Path.Join(structDirectory, x))
			.Select(x => ParseMethodHtml(x))
	...
	public static ConstantDeclaration ParseConstant(string filePath)
	{
		var xml = LoadHtml(filePath);
		var decl = xml.XPathSelectElement("body//h4[contains(text(), 'Declaration')]/..//pre").Value.Trim();
		var declName = Path.GetFileNameWithoutExtension(filePath).Replace("const.", "");
		var declValue = Regex.Match(decl, @"#define ([^ ]+) (.*)").Groups[2].Value;
